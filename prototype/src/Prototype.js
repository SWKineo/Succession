...
article.getSubmission = (version) => {
  for (let i = 0; i < article.submissions.length; i++) {
      if (article.submissions[i].version == version)
          return article.submissions[i]
  }

  return null
}

article.getCritique = (submission, critiqueId) => {
  for (let i = 0; i < submission.critiques.length; i++) {
    if (submission.critiques[i].id == critiqueId)
        return submission.critiques[i]
  }
